...
template<typename T>
bool get_ros_param(const ros::NodeHandle& nh, std::string param_name, T& result, T& default_value) {
    if (nh.getParam(param_name, result) == false) {
        ROS_WARN("Parameter %s does not exist. Defaulting to %s", param_name, std::string(default_value).c_str());
